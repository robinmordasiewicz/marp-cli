...
cat deployment.yaml | sed -re "s/image:[[:space:]]robinhoodis\/marp:.*/image: robinhoodis\/marp:${CONTAINERVERSION}/" > deployment.yaml.tmp && mv deployment.yaml.tmp deployment.yaml
